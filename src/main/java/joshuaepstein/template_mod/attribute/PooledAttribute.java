...
public abstract class PooledAttribute<T> extends TAttribute.Instance<T> {

  protected PooledAttribute() {

  }

  protected PooledAttribute(TAttribute.Modifier<T> modifier) {
    super(modifier);
  }

  public static abstract class Generator<T, O extends Generator.Operator<T>> implements TAttribute.Instance.Generator<T> {
  ...
  public static class Pool<T, O extends Pool.Operator<T>> {
    ...
    @Expose public List<Entry<T, O>> entries = new ArrayList<>();
    ...
    public T generate(Random random) {
      ...
      for(int i = 0; i < roll; i++) {
        Entry<T, O> entry = this.getRandom(random);
